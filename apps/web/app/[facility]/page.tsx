...
export default async function FacilityPage({ params }: Props) {
  ...
  return <>
    ...
    <main id="main" tabIndex={-1}>
      ...
      {!f.openingNotice && <section id="prova-gratis" aria-labelledby="trial-heading" className="border-y border-line bg-surface py-16 sm:py-20"><div className="page-width grid gap-10 md:grid-cols-2">
        <div><p className="eyebrow text-muted">Prova IW nära {f.name}</p><h2 id="trial-heading" className="mt-4 text-3xl font-semibold tracking-tight sm:text-4xl">Första passet bjuder vi på.</h2><p className="body-copy mt-6">{siteConfig.trial}</p><Link className="button button-light mt-7" href="/medlemskap#starta">Välj gratis provträning</Link></div>
      ...
      <div className="page-width"><Section id="filosofi" number="02" title="Mycket gym. Mycket omtanke."><p className="body-copy">{f.openingNotice ? siteConfig.philosophy.replace("Kom och prova – vi vill att du ska känna skillnaden själv.", "Vi ser fram emot att välkomna dig efter öppningen.") : siteConfig.philosophy}</p></Section></div>
      <section id="medlemskap" className="border-y border-line bg-surface py-16 sm:py-20" aria-labelledby="membership-heading"><div className="page-width"><p className="eyebrow text-muted">Medlemskap · {f.name}</p><h2 id="membership-heading" className="mt-4 text-3xl font-semibold tracking-tight sm:text-4xl">Hitta ditt medlemskap.</h2><div className={`mt-9 grid gap-4 ${f.memberships.length > 1 ? "lg:grid-cols-3" : "max-w-2xl"}`}>{f.memberships.map(m => <article key={m.name} className="membership-card"><h3 className="text-lg font-medium">{m.name}</h3><p className="mt-7 text-3xl font-semibold">{m.price}</p><p className="mt-4 text-sm leading-6 text-muted">{m.detail}</p><Link href={f.openingNotice ? "#forboka" : "/medlemskap#starta"} className="button button-light mt-8 w-full">{f.openingNotice ? "Förboka medlemskap – 299 kr/mån" : "Välj medlemskap"}</Link></article>)}</div></div></section>
      {f.openingNotice && <section id="forboka" className="prebooking py-14 sm:py-20" aria-labelledby="prebooking-heading"><div className="page-width grid gap-10 md:grid-cols-2"><div><p className="eyebrow text-[#80ff00]">Var med från starten</p><h2 id="prebooking-heading" className="mt-4 text-3xl font-semibold">Förboka medlemskap – 299 kr/mån</h2><p className="body-copy mt-5">Lämna bara ditt namn och mobilnummer. Vi kontaktar dig inför öppningen av IW Nära Skönsmon i november 2026. Ingen betalning görs nu.</p></div><ContactRequest purpose="skonsmon" emailEnabled={Boolean(process.env.RESEND_API_KEY && process.env.MAIL_FROM)} /></div></section>}
      <FacilityFaq facility={f} />
      <div className="page-width"><Section id="hitta-hit" number="03" title={`Välkommen till ${f.name}.`}><p className="body-copy mb-8">{f.openingNotice ? "Hör av dig med frågor om öppningen eller förbokningen. Gymmet är under konstruktion och tar ännu inte emot besök." : "Hör av dig för att prova gymmet, få hjälp att komma igång eller prata om ditt medlemskap. Vi kommer överens om en tid för en personlig genomgång."}</p><dl className="grid gap-8 sm:grid-cols-2"><div><dt className="detail-label">Adress</dt><dd className="body-copy">{f.address}<a className="nav-link mt-3 block text-sm" href={mapUrl}>Visa vägen i Google Maps ↗</a></dd></div><div><dt className="detail-label">Öppettider</dt><dd className="body-copy">{f.hours}</dd></div><div><dt className="detail-label">Kontakt</dt><dd className="body-copy">{f.email ? <a className="nav-link" href={`mailto:${f.email}?subject=${encodeURIComponent(`Medlemskap / frågor – ${f.name}`)}`}>{f.email}</a> : null}<div className="mt-4 space-y-3">{siteConfig.contacts.map(contact => <a key={contact.phone} className="nav-link flex min-h-12 flex-col justify-center" href={`tel:+46${contact.phone.replace(/[^0-9]/g, "").slice(1)}`}><span className="font-medium text-foreground">{contact.phone}</span><span className="ml-2 text-sm text-muted">({contact.name} – {contact.role})</span></a>)}</div></dd></div></dl></Section></div>
